...
import com.demo.compaction.Compactor.FileFormat.FileFormat
...
class ParquetCompactor extends Compactor {
  override protected def compactInternal(conf: Configuration, compactFrom: Array[String], compactTo: String): Unit = {
    ...
    val writer = new ParquetFileWriter(conf, mergedMeta.getSchema, new Path(compactTo), ParquetFileWriter.Mode.CREATE, ParquetWriter.DEFAULT_BLOCK_SIZE, ParquetWriter.MAX_PADDING_SIZE_DEFAULT)
