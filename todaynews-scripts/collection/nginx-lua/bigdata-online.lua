...
local broker_list = {
	{ host = "10.81.2.22", port = 9092 },
	{ host = "10.81.2.23", port = 9092 },
	{ host = "10.81.2.24", port = 9092 }
}
...
if "/bigdata/app/url" == uri and "POST" == request_method then
    --转换字符串为json对象
	local ok, err = bp:send("app-url", nil, ngx.req.get_body_data())
   if not ok then
...
elseif "/bigdata/app/button" == uri and "POST" == request_method then
    ...
    for k, v in pairs(messages["data"]) do
        local ok, err = bp:send("app-button", nil, cjson.encode(v))
        if not ok then
